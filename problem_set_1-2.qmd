...
```         
So according to skim data, the Some post-high school, no bachelor’s degree is the most common highest level of education
```
...
The most popular level is "Most of the time"
...
Every value larger than `r mean(iq) + 1.5 * sd(iq)` and smaller than`r`mean(iq) - 1.5 \* sd(iq)\` is an outlier,

so the outliers are `r iq [iq >= quantile(iq, .75) + 1.5 * sd(iq) ]` and `r iq [iq <= quantile(iq, .25) - 1.5 * sd(iq) ]`
...
From the picture, we can see that compare with the normal distribution, the distribution among these students are more skew to lift, and more concentrates to the middle.
...
Categorical: Race, Age, party affiliatoin
